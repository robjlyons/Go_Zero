...
class MuZeroTest(unittest.TestCase):
    ...
    def test_training_populates_replay_and_has_finite_loss(self):
        self.agent.train(1, seed=7, log_path=None)
    ...
    def test_training_uses_a_hard_environment_step_budget(self):
        with mock.patch("MuZero_Simple.step_env", wraps=step_env) as counted_step:
            self.agent.train(5, seed=7, log_path=None)
    ...
    def test_training_rejects_non_positive_step_budget(self):
        with self.assertRaisesRegex(ValueError, "max_steps must be positive"):
            self.agent.train(0, seed=7, log_path=None)

    def test_training_exports_jsonl_performance_metrics(self):
        with tempfile.TemporaryDirectory() as directory:
            log_path = Path(directory) / "metrics.jsonl"
            self.agent.train(2, seed=7, log_path=log_path)
            records = [json.loads(line) for line in log_path.read_text().splitlines()]

        self.assertEqual(len(records), 1)
        self.assertEqual(records[0]["environment_steps"], 2)
        self.assertEqual(records[0]["collected_steps"], 2)
        self.assertEqual(records[0]["max_steps"], 2)
        self.assertGreater(records[0]["steps_per_second"], 0.0)
        self.assertIn("rollout_seconds", records[0])
